...
export class WaterlevelhistoryComponent implements OnInit {
  ...
  private platformPauseSubscription: Subscription;
  ...
  updateChart() {
    var parsedData: any = [];
    parsedData = this.correctChartDataSet.map((dataset) =>
      Math.floor(dataset.average * this.datasetMultiplier)
    );
    this.correctChartLabels = this.correctChartDataSet.map(
  ...
  getData() {
    ...
    this.apiService.customGetRequest(endpointUrl + this.chartUrl).subscribe({
      next: (data) => {
        // console.log('response data getdiadata: ' + JSON.stringify(data))
        this.correctChartDataSet = data;
        this.selectedTimeframeWaterlevel = this.selectedTimeframeWaterlevel[0]
          ? this.selectedTimeframeWaterlevel[0]
          : this.selectedTimeframeWaterlevel;
        this.updateChart();
        var parsedData: any = [];
        parsedData = this.correctChartDataSet.map((dataset) =>
          Math.floor(dataset.average * this.datasetMultiplier)
  ...
  ngOnInit() {
    ...
    this.checkConnection = interval(60000).subscribe(() => {
      this.getData();
    });
